...
const StoriesBar = () => {
  ...
  const [showModal, setShowModel] = useState(false)   // controls "create new story" modal
  const [viewStory, setViewStory] = useState(null)    // controls "view story" overlay
  ...
  return (
    <div className="w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4">
      <div className="flex gap-4 pb-5">
        {/* Add Story Card */}
        <div
          onClick={() => setShowModel(true)} // open create story modal
          className="rounded-lg shadow-sm min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 border-2 border-dashed border-indigo-300 bg-gradient-to-b from-indigo-50 to-white"
        >
        ...
        {stories.map((story, index) => (
          <div
            key={index}
            onClick={() => setViewStory(story)} // open story overlay
